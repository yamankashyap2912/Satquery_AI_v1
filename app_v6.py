...
class _ReBENClassifier(torch.nn.Module):
    """Self-contained stand-in for `reben_publication.BigEarthNetv2_0_ImageClassifier`."""
    def __init__(self, hf_config: dict):
        super().__init__()
        ilm_config = ILMConfiguration(
            timm_model_name=hf_config["timm_model_name"],
            hf_model_name=hf_config.get("hf_model_name"),
            classes=hf_config["classes"],
            channels=hf_config["channels"],
            image_size=hf_config["image_size"],
            network_type=ILMType.IMAGE_CLASSIFICATION,
            drop_rate=hf_config.get("drop_rate", 0.2),
            load_pretrained_timm_if_available=False,
            load_pretrained_hf_if_available=False,
        )
        self.model = ConfigILM(ilm_config)
    ...
    def from_pretrained(cls, repo_id: str, device: str = "cpu"):
        ...
        if missing or unexpected:
            raise RuntimeError(
                "Checkpoint keys didn't line up with the rebuilt architecture — "
...
def load_s2_classifier():
    """The BEN-19 scene classifier — used for the grounding tab and as scene-context evidence."""
    try:
        m = _ReBENClassifier.from_pretrained("BIFOLD-BigEarthNetv2-0/resnet50-s2-v0.2.0", device=device)
        return m
    except Exception as e:
        st.session_state["s2_classifier_error"] = f"{type(e).__name__}: {e}"
        return None
...
def humanize_answer(question: str, answer_text: str, confidence: float) -> str:
    ...
    # Generic format output (good for bounding boxes or direct string answers)
    return f"The model's best guess is **{answer_text}** — with {confidence_pct:.1f}% confidence."
...
def run_grounding(s2_arrays: dict, class_name: str):
    ...
    # Robustly detect the target layer location for Grad-CAM
    if hasattr(s2_classifier, "model") and hasattr(s2_classifier.model, "vision_encoder"):
        target_layers = [s2_classifier.model.vision_encoder.layer4[-1]]
    elif hasattr(s2_classifier, "model") and hasattr(s2_classifier.model, "layer4"):
        target_layers = [s2_classifier.model.layer4[-1]]
    else:
        target_layers = [s2_classifier.layer4[-1]]
    ...
    if st.button("🚀 Run AI Analysis", type="primary", key="vqa_run"):
        if not s1_files or not s2_files:
            ...
            if len(s1_map) < 2 or len(s2_map) < 10:
                ...
                # Use the clean green banner without low confidence conditional styling
                banner_class = "answer-banner"
...
with tab_ground:
    ...
    if s2_classifier is None:
        real_error = st.session_state.get("s2_classifier_error", "unknown error — check the server logs")
        st.error(f"Scene classifier failed to load — grounding requires it.\n\n**Actual error:** `{real_error}`")
    else:
